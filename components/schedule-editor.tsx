...
import { CalendarDays, Check, Plus, Undo2, X, Copy, Trash2, MousePointer2, AlertCircle, Download, Upload } from 'lucide-react';
...
import { CATEGORIES, DAYS, DAY_CODES, START, END, STEP, STORAGE_KEY, timeLabel, parseTime, hoursLabel, layoutDay, validateEvent, overlaps, clamp, snap, moveEvent, resizeEvent, type ScheduleEvent, type Category } from '@/lib/schedule';
import { createProject, decodeProject, encodeProject, PROJECT_STORAGE_KEY, MAX_PROJECT_BYTES, type ProjectDocument, type GridLayout } from '@/lib/project';
...
function loadInitialSchedule() {
  try {
    const raw = localStorage.getItem(PROJECT_STORAGE_KEY) ?? localStorage.getItem(STORAGE_KEY);
...
export function ScheduleEditor() {
  ...
  useEffect(() => {
    if (!ready) return;
    return registerScheduleTools({ getEvents: () => projectRef.current.events, replaceEvents: next => flushSync(() => { commit(next); setDraft(null); setSelectedId(null); }) });
  ...
  return <div className="app-shell" style={{ '--hour-height': `${project.layout.hourHeight}px`, '--day-width': `${project.layout.dayWidth}px`, '--event-font-size': `${project.layout.fontSize}px` } as CSSProperties}>
                ...
                <div className={`day-columns ${dragPreview ? 'is-dragging' : ''}`} ref={grid} onPointerMove={movePointer} onPointerUp={endPointer} onPointerCancel={cancelGesture} onLostPointerCapture={() => { if (gesture.current) cancelGesture(); }}>
                  {DAYS.map((day, i) => <div className="day-column" key={day}>
                    ...
                    {layoutDay(displayedEvents, i).map(item => {
                      const compact = (item.end - item.start) / 60 * project.layout.hourHeight < project.layout.fontSize * 1.35 + 34;
